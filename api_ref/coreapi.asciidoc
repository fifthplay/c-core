...
1. Initializing with a custom uuid:

```
pubnub_t *ctx = pubnub_alloc();
pubnub_init("demo", "demo");
pubnub_set_uuid(ctx, "my_custom_uuid");
```
...
void _callback(pubnub_t *pb, enum pubnub_trans trans, enum pubnub_res result, void *user_data)
{
    ...
    case PBTT_PUBLISH:
        printf("Publish %d\n", result);
        printf("Published! Response from Pubnub: %s , Timetoken = %s\n", 
            pubnub_last_publish_result(pb),pubnub_last_time_token(pb));
        break;
    default:
...
int main()
{
    ...
    pubnub_register_callback(pbp, _callback, &user_data);

    res = pubnub_publish(pbp, chan, "\"Hello from Pubnub C-core docs!\"");
    if (res != PNR_STARTED) {
...
int main()
{
    ...
    res = pubnub_publish(pbp, chan, "\"Hello from Pubnub C-core docs!\"", true, true);
    if (res != PNR_STARTED) {
        printf("pubnub_publish() returned unexpected: %d\n", res);
        pubnub_free(pbp);
        return -1;
...
void _callback(pubnub_t *pb, enum pubnub_trans trans, enum pubnub_res result, void *user_data)
{
    ...
    switch (trans) {
    case PBTT_SUBSCRIBE:
        printf("Subscribed! Response from Pubnub: %s, Timetoken = %s\n", 
            pubnub_get(pb), pubnub_last_time_token(pb));
        break;
    default:
        printf("None?! result: %d\n", result);
        break;
...
(3)  Subscribing to a Presence channel: 
...
pubnub_init(ctx, “demo”, “demo”);
pubnub_subscribe(ctx, “hello_world-pnpres”, NULL);
pbresult = pubnub_await(ctx);
if (pbresult != PNR_OK) {
    printf(“Failed to subscribe, error %d\n”, pbresult);
    pubnub_free(ctx);
...
===== DESCRIPTION
Get the message history for the channel. This actually means "initiate a history transaction/operation".

If transaction is successful, the gotten messages will be available via the pubnub_get() which will give you exactly three messages (or, rather, elements). The first will be a JSON array of gotten messages, and the second and third will be the timestamps of the first and the last message from that array.
