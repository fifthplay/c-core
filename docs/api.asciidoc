...
|p        |pubnub_t | Yes | The Context to initialize (use pubnub_get_ctx() to obtain it)
|publish_key  | char * | | The string of the key to use when publishing messages
|subscribe_key  | char * | | The string of the key to use when subscribing to messages
